...
class Configuration:
    ...
    def analyze_quit(self, exitcode):
        glob.Debug.debug('[DEBUG] Application is exiting, checking if there is a {QUIT} configuration...')
        return self.analyze_special('QUIT', exitcode)

    def analyze_startup(self):
        glob.Debug.debug('[DEBUG] Application is starting up, checking if there is a {STARTUP} configuration...')
    ...
    def analyze_special(self, action, exitcode=None):
        ...
        for config in self.configs:
            if config['name'] == action_label:
                glob.Debug.debug('[DEBUG] '+action_label+' configuration found, will display notification if exists...')
                groups = []
                history_groups = []
                ...
                if on_exitcode_val is not None:
                    glob.Debug.debug('[DEBUG] Trigger notification if ' + ('not ' if not if_exitcode else ' ') + str(exitcode) + ' == ' + str(on_exitcode))
                    if exitcode_match:
                        glob.Debug.debug('[DEBUG] Exit code condition matched, will trigger this notification')
    ...
    def get_hanging_times(self):
        ...
        for config in self.configs:
            if config['name'] == '{HANGING}':
                ...
                glob.Debug.debug('[DEBUG] HANGING configuration found, time: '+str(config['timeout']))
        return hanging_times

    def analyze_hanging_with_timeout(self, threshold, last_lines):
        glob.Debug.debug('[DEBUG] Scanning for HANGING configurations that should trigger after '+str(threshold)+'ms...')
        for config in self.configs:
            if config['name'] == '{HANGING}' and threshold == config['timeout']:
                glob.Debug.debug('[DEBUG] Found configuration: '+str(config))
                notify_hanging = True
                if 'pattern' in config:
                    ...
                    glob.Debug.debug('[DEBUG] Hanging ' + ('if' if is_if else 'if not') + ' "'+pattern+'" in line: '+str(line_number_to_analyze) + ' "'+str(line_to_analyze)+'"')
                    ...
                    if (is_if and (matches is not None and len(matches) > 0)) or (not is_if and (matches is None or len(matches) == 0)):
                        ...
                        glob.Debug.debug('[DEBUG] HANGING configuration did not meet the pattern, will not display notification...')

                if notify_hanging:
                    glob.Debug.debug('[DEBUG] HANGING configuration with passed timeout found, will display notification if exists...')
                    groups = []
                    history_groups = []
                    notification = config['notification'] if 'notification' in config else None
                    self.send_notification(notification, groups, history_groups, 0)
    ...
    def analyze(self, line, accumulated=None):
        ...
        glob.Debug.debug('[DEBUG] Check if line is triggering a notification: "'+line+'"')
        for config in self.configs:
            if not config['name'].startswith('{') and not config['name'].endswith('}'):
                ...
                if pattern is not None:
                    ...
                    if groups is not None and len(groups) > 0:
                        ...
                        glob.Debug.debug('[DEBUG] Pattern is matching: "'+pattern+'"')
    ...
    def send_notification(self, original_notification, groups, history_groups, run_count):
        ...
        if notification is not None:
            ...
            glob.Debug.debug('[DEBUG] Notification found: ')
            for key in notification:
                glob.Debug.debug('[DEBUG] key='+key)
                ...
                glob.Debug.debug('[DEBUG] value='+value)
                notification[key] = value
            glob.Debug.debug('[DEBUG] Pushing notification: '+str(notification))

            # No sound option?
            notification['sound'] = notification_sound
            if glob.GlobalParams.is_no_sound():
                notification['sound'] = '{none}'
                glob.Debug.debug('[DEBUG] No sound will be played, as --no-sound have been set')
        ...
        if notification_sound != '{none}':
            if glob.Platform.is_linux():
                ...
                if not aplay_supported:
                    glob.Debug.debug('[DEBUG] Playing sounds together with the notification is not supported in your system')
                else:
                    extra_functions.CommandHelper.run_command_async(['aplay', notification_sound])
                    glob.Debug.debug('[DEBUG] Will try and play sound "'+notification_sound+'" through aplay')
            elif glob.Platform.is_windows():
                ...
                if not sounder_supported:
                    ...
                    glob.Debug.debug('[DEBUG] Will try and play sound "'+notification_sound+'" through sounder')
                ...
                if glob.GlobalParams.prefer_growl() or glob.GlobalParams.use_afplay():
                    ...
                    if not afplay_supported:
                        ...
                        glob.Debug.debug('[DEBUG] Will try and play sound "'+notification_sound+'" through afplay')
                else:
                    glob.Debug.debug('[DEBUG] Mac will play sound through the standard Sound Effects')
